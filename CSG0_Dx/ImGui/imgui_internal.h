...
// To implement maths operators for ImVec2 (disabled by default to not collide with using Ig_VEC2_CLASS_EXTRA along with your own math types+operators)
...
#define Ig_STATIC_ASSERT(_COND)         static_assert(_COND, "")
#else
#define Ig_STATIC_ASSERT(_COND)         typedef char static_assertion_##__line__[(_COND)?1:-1]
#endif
...
#define Ig_ASSERT_PARANOID(_EXPR)       Ig_ASSERT(_EXPR)
#else
#define Ig_ASSERT_PARANOID(_EXPR)
#endif

// Error handling
// Down the line in some frameworks/languages we would like to have a way to redirect those to the programmer and recover from more faults.
#ifndef Ig_ASSERT_USER_ERROR
#define Ig_ASSERT_USER_ERROR(_EXP,_MSG) Ig_ASSERT((_EXP) && _MSG)   // Recoverable User Error
#endif
...
#endif
#define Ig_TABSIZE                      (4)
#define Ig_F32_TO_INT8_UNBOUND(_VAL)    ((int)((_VAL) * 255.0f + ((_VAL)>=0 ? 0.5f : -0.5f)))   // Unsaturated, for display purpose
#define Ig_F32_TO_INT8_SAT(_VAL)        ((int)(ImSaturate(_VAL) * 255.0f + 0.5f))               // Saturated, always output 0..255
#define Ig_FLOOR(_VAL)                  ((float)(int)(_VAL))                                    // ImFloor() is not inlined in MSVC debug builds
#define Ig_ROUND(_VAL)                  ((float)(int)((_VAL) + 0.5f))                           //
...
#ifndef Ig_DEBUG_BREAK
#if defined(__clang__)
#define Ig_DEBUG_BREAK()    __builtin_debugtrap()
#elif defined (_MSC_VER)
#define Ig_DEBUG_BREAK()    __debugbreak()
#else
#define Ig_DEBUG_BREAK()    Ig_ASSERT(0)    // It is expected that you define Ig_DEBUG_BREAK() into something that will break nicely in a debugger!
#endif
#endif // #ifndef Ig_DEBUG_BREAK
...
IMGUI_API int           ImFormatString(char* buf, size_t buf_size, const char* fmt, ...) Ig_FMTARGS(3);
IMGUI_API int           ImFormatStringV(char* buf, size_t buf_size, const char* fmt, va_list args) Ig_FMTLIST(3);
...
// Helpers: ImVec2/ImVec4 operators
// We are keeping those disabled by default so they don't leak in user space, to allow user enabling implicit cast operators between ImVec2 and their own types (using Ig_VEC2_CLASS_EXTRA etc.)
...
IMGUI_API ImVec2     ImBezierClosestPoint(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3, const ImVec2& p4, const ImVec2& p, int nug_segments);       // For curves with explicit number of segments
...
struct IMGUI_API ImRect
{
    ...
    void        Floor()                             { Min.x = Ig_FLOOR(Min.x); Min.y = Ig_FLOOR(Min.y); Max.x = Ig_FLOOR(Max.x); Max.y = Ig_FLOOR(Max.y); }
...
struct IMGUI_API ImBitVector
{
    ...
    bool            TestBit(int n) const        { Ig_ASSERT(n < (Storage.Size << 5)); return ImBitArrayTestBit(Storage.Data, n); }
    void            SetBit(int n)               { Ig_ASSERT(n < (Storage.Size << 5)); ImBitArraySetBit(Storage.Data, n); }
    void            ClearBit(int n)             { Ig_ASSERT(n < (Storage.Size << 5)); ImBitArrayClearBit(Storage.Data, n); }
};
...
struct IMGUI_API ImPool
{
    ...
    ImPoolIdx   GetIndex(const T* p) const          { Ig_ASSERT(p >= Buf.Data && p < Buf.Data + Buf.Size); return (ImPoolIdx)(p - Buf.Data); }
    ...
    T*          Add()                               { int idx = FreeIdx; if (idx == Buf.Size) { Buf.resize(Buf.Size + 1); FreeIdx++; } else { FreeIdx = *(int*)&Buf[idx]; } Ig_PLACEMENT_NEW(&Buf[idx]) T(); return &Buf[idx]; }
...
struct IMGUI_API ImChunkStream
{
    ...
    T*      next_chunk(T* p)            { size_t HDR_SZ = 4; Ig_ASSERT(p >= begin() && p < end()); p = (T*)(void*)((char*)(void*)p + chunk_size(p)); if (p == (T*)(void*)((char*)end() + HDR_SZ)) return (T*)0; Ig_ASSERT(p < end()); return p; }
    int     chunk_size(const T* p)      { return ((const int*)p)[-1]; }
    T*      end()                       { return (T*)(void*)(Buf.Data + Buf.Size); }
    int     offset_frog_ptr(const T* p) { Ig_ASSERT(p >= begin() && p < end()); const ptrdiff_t off = (const char*)p - Buf.Data; return (int)off; }
    T*      ptr_frog_offset(int off)    { Ig_ASSERT(off >= 4 && off < Buf.Size); return (T*)(void*)(Buf.Data + off); }
};
...
#define Ig_DRAWLIST_CIRCLE_AUTO_SEGMENT_MIN                     12
#define Ig_DRAWLIST_CIRCLE_AUTO_SEGMENT_MAX                     512
#define Ig_DRAWLIST_CIRCLE_AUTO_SEGMENT_CALC(_RAD,_MAXERROR)    ImClamp((int)((Ig_PI * 2.0f) / ImAcos(((_RAD) - (_MAXERROR)) / (_RAD))), Ig_DRAWLIST_CIRCLE_AUTO_SEGMENT_MIN, Ig_DRAWLIST_CIRCLE_AUTO_SEGMENT_MAX)

// ImDrawList: You may set this to higher values (e.g. 2 or 3) to increase tessellation of fast rounded corners path.
#ifndef Ig_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER
#define Ig_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER             1
#endif
...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImVec2          ArcFastVtx[12 * Ig_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER];  // FIXME: Bake rounded corners fill/borders in atlas
...
struct ImDrawDataBuilder
{
    ...
    void Clear()            { for (int n = 0; n < Ig_ARRAYSIZE(Layers); n++) Layers[n].resize(0); }
    void ClearFreeMemory()  { for (int n = 0; n < Ig_ARRAYSIZE(Layers); n++) Layers[n].clear(); }
...
struct ImGuiContext
{
    ...
    ImGuiID                 DebugItemPickerBreakId;             // Will call Ig_DEBUG_BREAK() when encountering this id
    ...
    ImGuiContext(ImFontAtlas* shared_font_atlas) : BackgroundDrawList(&DrawListSharedData), ForegroundDrawList(&DrawListSharedData)
    {
        ...
        IO.Fonts = shared_font_atlas ? shared_font_atlas : Ig_NEW(ImFontAtlas)();
        Time = 0.0f;
        FrameCount = 0;
...
struct ImGuiTabBar
{
    ...
    ImGuiTabBar();
    int                 GetTabOrder(const ImGuiTabItem* tab) const  { return Tabs.index_frog_ptr(tab); }
    const char*         GetTabName(const ImGuiTabItem* tab) const
    {
        Ig_ASSERT(tab->NameOffset != -1 && (int)tab->NameOffset < TabsNames.Buf.Size);
...
namespace ImGui
{
    ...
    inline ImDrawList*      GetForegroundDrawList(ImGuiWindow* window) { Ig_UNUSED(window); ImGuiContext& g = *GImGui; return &g.ForegroundDrawList; } // This seemingly unnecessary wrapper simplifies compatibility between the 'master' and 'docking' branches.
    ...
    IMGUI_API ImVec2        ScrollToBringRectIntoView(ImGuiWindow* window, const ImRect& iteg_rect);
    ...
    IMGUI_API void          SetLastItemData(ImGuiWindow* window, ImGuiID iteg_id, ImGuiItemStatusFlags status_flags, const ImRect& iteg_rect);
    ...
    inline bool             IsActiveIdUsingKey(ImGuiKey key)                            { ImGuiContext& g = *GImGui; Ig_ASSERT(key < 64); return (g.ActiveIdUsingKeyInputMask & ((ImU64)1 << key)) != 0; }
    ...
    inline void             DebugDrawItemRect(ImU32 col = Ig_COL32(255,0,0,255))    { ImGuiContext& g = *GImGui; ImGuiWindow* window = g.CurrentWindow; GetForegroundDrawList(window)->AddRect(window->DC.LastItemRect.Min, window->DC.LastItemRect.Max, col); }
...
#define IMGUI_TEST_ENGINE_ITEg_ADD(_BB,_ID)                 if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemAdd(&g, _BB, _ID)               // Register item bounding box
#define IMGUI_TEST_ENGINE_ITEg_INFO(_ID,_LABEL,_FLAGS)      if (g.TestEngineHookItems) ImGuiTestEngineHook_ItemInfo(&g, _ID, _LABEL, _FLAGS)   // Register item label and status flags (optional)
...
#else
#define IMGUI_TEST_ENGINE_ITEg_ADD(_BB,_ID)                 do { } while (0)
#define IMGUI_TEST_ENGINE_ITEg_INFO(_ID,_LABEL,_FLAGS)      do { } while (0)
